...
- 完成教程中所有代码后，可能会发现持续打印出 `It should shutdown!` 信息，并出现各种乱码。这主要是因为`sbi 标准`已经更改，原代码不再适用。我们已经在 [这里](https://gitee.com/lispking/rCore-Tutorial-v3/blob/master/ch1/os/src/sbi.rs) 提供了新版本的代码，并将相关代码上传到了 [ch1 目录](https://gitee.com/lispking/rCore-Tutorial-v3/tree/master/ch1/os)。执行`make run`后，若看到特定的效果，那就代表环境准备完成了。
