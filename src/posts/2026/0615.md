...
date: '2026-06-15'
description: '"线上 P99 延迟飙了，日志里什么都没有。"'
tags: ['Rust', '可观测性', 'tracing', 'Metrics', 'OpenTelemetry']
---
...
microservices 文章里已经提过这个，但我想展开讲一下——因为很多人觉得"开 debug 级别只是多点日志"。

**不是的。开 debug 级别的代价比你想的大得多：**

1. **磁盘 I/O 暴增**：debug 日志量可能是 info 的 10-100 倍
2. **CPU 开销**：即使没有 subscriber，`tracing` 也要评估 `Interest`（虽然很快，但不是零）
3. **内存压力**：JSON 格式化、大字段序列化都在堆上分配
4. **关键信息被淹没**：出了问题你要在海量 debug 输出里找 error
...
`TraceLayer` 自动为每个 HTTP 请求创建 Span，记录 method、uri、status_code。`PropagateRequestIdLayer` 自动在请求头里注入/提取 `x-request-id`。
...
1. **初始化**：`init_tracing()` 返回 `SdkTracerProvider`（前面已经给过代码）
2. **路由层**：加上 `TraceLayer` + `PropagateRequestIdLayer`
3. **优雅关闭**：`provider.shutdown()` flush 遥测数据
...
│                                                     │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────┐   │
│  │ tracing  │  │ metrics  │  │ tracing-error    │   │
│  │(Events + │  │(Counter/ │  │ (SpanTrace)      │   │
│  │ Spans)   │  │Histogram)│  │                  │   │
│  └────┬─────┘  └────┬─────┘  └────────┬─────────┘   │
│       │              │                 │             │
│  ┌────┴──────────────┴─────────────────┴──────────┐  │
...
│  │  ┌─────────┐ ┌──────────┐ ┌─────────────────┐  │  │
│  │  │fmt Layer│ │OTel Layer│ │metrics-tracing  │  │  │
│  │  │ (JSON)  │ │          │ │  -context       │  │  │
│  │  └────┬────┘ └────┬─────┘ └───────┬─────────┘  │  │
│  └───────┼───────────┼───────────────┼────────────┘  │
│          │           │               │               │
